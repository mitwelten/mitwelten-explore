...
initial_page_items = []
for depl_cards in [
    env_deployment_select_cards(d, id_role=ids.add_trace)
    for d in get_deployments(type_query="env.")
]:
...
def update_dataset_list(deployment_id):
    ...
    if deployment_id:
        for depl_cards in [
            env_deployment_select_cards(d, id_role=ids.add_trace)
            for d in get_deployments(type_query="env.", deployment_id=deployment_id)
        ]:
            cards += depl_cards
        return cards
    else:
