...
def get_deployments(type_query = None, deployment_id = None):
    ...
    if res.status_code == 200:
        ...
        if type_query:
            filtered_deployments = [d for d in deployments if type_query in d.get("node").get("type").lower()]
...
def get_deployment_location(deployment_id):
    ...
    if deployment is None:
        return None
    return {"latitude":deployment.get("location").get("lat"),"longitude":deployment.get("location").get("lon")}